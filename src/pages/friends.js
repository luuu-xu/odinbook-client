...
function FriendRequestCard({ user }) {
  ...
  const handleAcceptFriendRequest = async () => {
    ...
    const res = await fetch(`http://localhost:8080/api/authuser/accept-friend-request/${user._id}`, {
      method: 'POST',
      headers: {
...
function UserCard({ user, friendRequestsSentIds }) {
  ...
  const handleAddFriend = async () => {
    setFriendRequestStatus('loading');
    const res = await fetch(`http://localhost:8080/api/authuser/send-friend-request/${user._id}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${session.accessToken}`,
        'Content-Type': 'application/json'
